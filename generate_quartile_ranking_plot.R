...
core_dataset <- list.files("results", pattern = "krsa_table.+.csv", full.names = TRUE) |>
  set_names(~ basename(.x) |> str_extract("krsa_table_(.*)_STK.csv", 1L)) |>
  map(~ read.csv(.x)) |>
  ...
  mutate(
    Label = str_remove(dataset, "krsa_table"),
