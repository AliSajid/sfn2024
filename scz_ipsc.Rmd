...
params:
  ...
  signal_file: "kinome_data/scz_ipsc/_KRSAsOutput_Median_SigmBg_230409171918.txt" # Change the file name to the one you have
  saturation_file: "kinome_data/scz_ipsc/_KRSAsOutput_Signal_Saturation_230409171919.txt" # Change the file name to the one you have
...
gendered_data <- krsa_qc_steps(raw_data) |>
  mutate(
    Group = case_when(
      SampleName == "F_CNTRL" ~ "CTL_F",
      SampleName == "F_SCZ" ~ "SCZ_F",
      SampleName == "M_CNTRL" ~ "CTL_M",
      SampleName == "M_SCZ" ~ "SCZ_M"
    ),
...
pooled_data <- krsa_qc_steps(raw_data) |>
  mutate(
    Group = case_when(
      grepl("CNTRL", SampleName) ~ "CTL",
      grepl("SCZ", SampleName) ~ "SCZ"
    ),
    Barcode = case_when(
      grepl("^F", SampleName) ~ paste0(Barcode, "F"),
      grepl("^M", SampleName) ~ paste0(Barcode, "M")
    ), 
    SampleName = paste0(Group, "_", Barcode)
  )
...
- `r groups[[1]]` vs `r groups[[2]]`
- `r groups[[3]]` vs `r groups[[4]]`
- `r groups[[3]]` vs `r groups[[1]]`
- `r groups[[4]]` vs `r groups[[2]]`
